...
class OmniRobotController(Node):
    def __init__(self):
        ...
        self.Rw = 0.05  # Rayon des roues (en mètres)
    ...
    def cmd_vel_callback(self, msg):
        ...
        self.wheel_vel[1] = (vel_x * math.sin(math.pi / 4)            + vel_y * math.cos(math.pi / 4)            + self.L * vel_w) / self.Rw
        self.wheel_vel[0] = (vel_x * math.sin(math.pi / 4 + math.pi / 2) + vel_y * math.cos(math.pi / 4 + math.pi / 2) + self.L * vel_w) / self.Rw
        self.wheel_vel[3] = (vel_x * math.sin(math.pi / 4 - math.pi)   + vel_y * math.cos(math.pi / 4 - math.pi)   + self.L * vel_w) / self.Rw
...
"""
    try:
        ...
        omnirobot_controller.get_logger().info('Shutting down gracefully...')
    finally:
        if rclpy.ok():  # Vérifie si ROS 2 est encore actif
